...
def command_image(m):
    cid = m.chat.id
    bot.send_message(cid,
                     "Let's start. I'm able to download torrent file from link/meta or you can send it to me directly",
                     reply_markup=howDownloadTorrentFileSelect)  # show the keyboard
...
def return_to_zero(m):
    cid = m.chat.id
    bot.send_message(cid,
                     "Welcome again. I'm able to download torrent file from link/meta or you can send it to me directly",
                     reply_markup=howDownloadTorrentFileSelect)  # show the keyboard
    userStep[cid] = 1  # set the user to the next step (expecting a reply in the listener now)
...
def torrent_file_from_link(m):
    ...
    link = m.text
    bot.send_chat_action(cid, 'typing')
    match_link = re.search('http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+', link)
    if match_link:
...
def downloader(m, link):
    ...
    if os.path.exists(log_file):
        ...
        while True:
            ...
            load_info = ""
            text_log = os.popen(
                " tail -n 100 " + log_file + " | grep -E 'DL|(OK)|SEED' | tail -n 1 ")  # |ERR to errors updater
    ...
    if os.path.exists(files_folder):
        ...
        for file in files:
            if os.path.getsize(file) > 52428800:  # > 50Mb
                print "zZz big files.."
                bot.send_message(cid,
                                 "Sotty, not telegram doesn't allow send files more then 50Mb. We're working around")
